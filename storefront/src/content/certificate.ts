...
/**
 * What pressing a share control does to the answer {@link WHO_CAN_SEE} gives.
 * LD-11 J11.
 *
 * **The one action the page invites is the one that ends the unlisting.** G4
 ...
 * (`certificate-indexing.test.ts` renders it and looks). On a gift the name is
 * a third party's, and the preview is the first place it appears.
 *
 * "Starts", not "publishes": each link opens a composer the person still has
