...
export const PRIVACY: LegalDocument = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Three essential cookies can be set under this site's own domain. One is ours and two are Stripe's, and the two are set only if you go to the payment page.",
        ...
        "Stripe's are called __stripe_mid and __stripe_sid. They are set by Stripe's script under this site's own domain, and they identify the browser you are paying from so that Stripe can tell an ordinary payment from a fraudulent one. __stripe_sid lasts 30 minutes. __stripe_mid lasts a year.",
        ...
        "Google Analytics and Meta Pixel are optional measurement tools. They load only after you agree in the analytics choice below the footer. We send fixed funnel events, route categories, product handles, currency and amounts. We do not send the address or title of the page you are reading, URL queries, certificate serials, form entries, customer or order identifiers, or your words to Baldrick. The tools run in an isolated measurement frame with its own fixed page identity and no referring page address.",
        "The measurement frame cannot read this site's cookies or browser storage, and it sets no first-party analytics cookies. Google is configured with analytics and advertising storage disabled. Vendors still receive your network address and browser/device information when your browser contacts them, and may generate their own measurement identifiers. Your browser controls cookies on connections to vendor domains; those vendors' privacy notices describe their own use and retention. Optional measurement is not anonymous merely because we omit customer details.",
        "Refusing keeps the tools off. Stop analytics destroys the measurement frame, discards events waiting there and stops future collection from that frame; it cannot recall requests already sent or delete data the vendors already received. The one preference this site stores is lousydeal.analytics-consent.v1; it says only whether you agreed or refused. It has no automatic expiry: it remains until you change your choice, clear this site's browser storage, or your browser removes it. No interaction from before consent is saved for later replay.",
        "No error reporter, third-party font or content delivery network is used. The payment page loads Stripe, because that is what taking a card payment consists of; apart from Stripe and the optional Google and Meta tools you chose, this site is rendered on our own server and fetches nothing from anywhere else.",
    ...
    },
    {
      ...
      body: [
        "Our own code asks you for four things: your email address, the country you are in, the consent described in Refunds and Withdrawal, and — optionally — the two lines that go on the certificate itself. If your order contains something that has to be posted, it asks for a fifth: where to send it.",
        "The two certificate lines are a name and a dedication, both optional and **both public**: they are printed on a certificate anybody with its address can read, which the checkout tells you before you type them. They are the one thing here you give us deliberately for publication, and leaving them blank prints “the bearer” instead. Neither is your billing name, which we never ask for.",
    ...
    },
    {
      ...
      body: [
        "Five companies can handle data when you use this site, and these are all of them. Google and Meta do so only if you choose optional analytics. A sixth kind of party — whoever carries a parcel — is described at the end of this section.",
        "Google Analytics and Meta Pixel receive the measurement data described in §2 only after your opt-in. We use them to understand whether the shop's purchase path is usable. Google and Meta may process it outside the European Economic Area, including in the United States. Their respective privacy notices govern their own processing and retention; stopping collection here does not erase an existing vendor record.",
