...
function start(config = { googleTagId: "G-EXAMPLE", metaPixelId: "123456789" }) {
  const commands: unknown[][] = [];
  const parent = {};
  let listener: (event: unknown) => void = () => undefined;
  const context: Record<string, unknown> = {
    parent: Object.assign(parent, { postMessage: () => undefined }),
    document: { createElement: () => ({}), head: { append: () => undefined } },
  ...
  listener({ source: parent, data: { kind: "lousydeal.analytics.configure", ...config } });
  ...
  return { commands, meta, fbq, google, html, send: (data: unknown, source: unknown = parent) => listener({ data, source }) };
}
