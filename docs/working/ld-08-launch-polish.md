...
| Provider reporting | **Completed in LD-10.** The fixed aggregate reporting and social operating contract is recorded in [`provider-reporting.md`](../current/provider-reporting.md). That work does not alter this slice's consent-gated Google Analytics and Meta Pixel collection or its Buffer-only draft workflow. |
