...
| A shipping option a cart can select | **no** | the Printful provider is registered and priced, but nothing creates a stock location, fulfillment set, service zone, shipping profile or shipping option — so `listCartShippingOptions` answers with nothing and every parcel shows the unavailable notice. P7a was ticked as though it did. **P7b** |
