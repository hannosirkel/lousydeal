...
| Postage charged on the scale Medusa reads | **unknown, and suspected wrong by 100×** | `chargeForRate` returns minor units into a `calculated_amount` that every other amount on that path treats as major. A $5.22 rate would be charged as $663.00. **P7c**, settled by reading a real cart rather than by reasoning |
