...
| Next action | Decide whether launch requires all six Buffer profiles or the three the operator connected; then create one X draft through Meeme and delete that exact draft in Buffer's UI. Confirm Printful billing before removing only the live storefront Access gate. Separately rotate the exposed test-mode Stripe keys, disable the named Google/Meta automatic collection settings, and authorize only Gate F order #7's two original failed email rows if they should be recovered. Provision and verify live Stripe after the closed site is public, then complete F2 and the final evidence PR. |
| Blocked | **Closed public publication** waits only on operator confirmation of Printful billing before the scoped Access removal and unauthenticated closed-site verification; it does not require live Stripe readiness. **LD-08 completion** still requires the Buffer scope/draft decision, live Stripe runtime/key and webhook evidence, clean analytics account read-back, recovered Gate F mail and public verification. **Payment opening** is later and remains blocked on Stripe account acceptance plus an explicit `STORE_OPEN=true` promotion. Provider reporting remains deferred and non-blocking in [`LD-10`](./ld-10-provider-reporting.md). |
