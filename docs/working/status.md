...
| Merch exists in Medusa | **yes**, once `predeploy` runs | **P6b** wrote `MedusaMerchSeedTarget`, the `seed:merch` command and its `predeploy` entry, after `configure:commerce` so the shipping profile exists |
| A shipping option a cart can select | **yes**, once `predeploy` runs | the Printful provider is registered and priced, but nothing creates a stock location, fulfillment set, service zone, shipping profile or shipping option — which **P7b** built: a stock location linked to the sales channel, a fulfillment set, a worldwide service zone, the `merch` profile and a calculated option bound to the Printful provider |
| Postage charged on the scale Medusa reads | **yes** | was wrong by 100× and fixed in **P7c**; confirmed in **P7d** against a real cart — a $15 mug to Tallinn totals 21.93 with 6.93 of postage. Medusa builds a shipping option's `amount` from the pricing module for a flat option and from the provider for a calculated one, into the same field, so both are major units |
| Printful webhook subscribed, and its secret captured | **yes**, 2026-09-09 | Subscribed to the four events `webhook.ts` accepts, at the bypassed path on the test storefront; the once-shown hex `secret_key` went into `.keys/` at mode 0600, through OpenBao to the workloads. **Verified end to end**: a delivery signed with the deployment's own secret answers 200, a wrongly signed one 401 |
| Merch buyable end to end on the test deployment | **yes**, 2026-09-10 | the operator completed a checkout. Three faults in sequence, each hiding the next: the predeploy Job was never told Printful existed so it skipped the shipping option by design; telling it made `medusa db:migrate` want a directory a read-only filesystem refused; and the storefront then discarded the option for lacking a price a *calculated* option does not carry until it is attached. Measured after: `options listed: 1 Postage/calculated`, `ATTACHED postage = 6.48` |
| The two VAT counters are run | **operator action, periodically** | `npm run report:vat-thresholds` reports Union turnover against €100,000 and Latvian supplies against €50,000. Nothing schedules it, deliberately. Crossing the first must reach EMTA within 15 working days |
| Destination VAT rates verified against TEDB | **yes**, 2026-09-10 | all 27 agree, nothing changed. Verified through the Commission's **VAT Retrieval Service**, the machine-readable interface into TEDB, queried both at `situationOn = 2026-10-01` and across `2026-09-10 → 2026-12-31`, which would have surfaced a scheduled change; there is none. The four rates that moved since 2024 were each confirmed against the member state's own authority. **A pass is dated, not permanent**: TEDB is self-reported and this holds unless a member state legislates after 2026-09-10 |
| Special VAT territories routed correctly | **handled at reporting time**, decision `014` | not in the checkout, and deliberately: VAT is absorbed and never itemised, so a mis-routed territory corrupts the OSS return and no buyer's total. `014` carries the postcode table, keeps every destination open, and corrects at OSS preparation — a ten-minute hand scan until the script exists, and the first return is due January 2027. Almost every error is an **over-remit**; the two that are not (Northern Ireland goods, Monaco) go through the OSS registration already held |
| Northern Ireland | **answered**, decision `014` | Protocol Art 8 with Annex 3 applies the VAT Directive to NI **in respect of goods**: an EU-dispatched parcel is an intra-Community distance sale at the UK's 20%, declared with `XI` as member state of consumption. A **certificate** to a BT address is outside EU VAT entirely and falls into `013`'s open United Kingdom question. `BT` is exhaustive and exclusive for NI. One question for EMTA before the first BT parcel: confirm the e-MTA return accepts `XI` |
| Whether the €10,000 threshold was crossed earlier in 2026 | **operator check** | destination liability and OSS registration are different questions: crossing the Art 59c threshold makes destination VAT due from the supply that crossed it, registration or not. Nothing here has crossed anything — the shop has taken no live payment — but that, and not the registration date, is the misreporting trap |
