...
| Destination VAT rates verified against TEDB | **no** | the Commission discontinued its rates PDF after January 2021 and its OSS portal now designates the Taxes in Europe Database, which is an interactive application. `tax-model.ts`'s 27 rates rest on two agreeing 2026 compilations plus the instrument behind every rate that moved since 2024. **One manual TEDB pass before the publication gate** |
| Special VAT territories routed correctly | **no, and wrong in both directions** | Medusa routes on `country_code`, so the Canaries resolve as Spain, Åland as Finland, Heligoland as Germany — each **outside the EU VAT area** under Art 6, where the right answer is no EU VAT. Portugal needs 16% Azores / 22% Madeira against a 23% mainland; Greece reduces to 17% on some Aegean islands and whether a foreign distance seller may is unresolved (24% over-remits, which is the safe direction). A row of its own |
| Northern Ireland | **no** | goods delivered there are inside the EU VAT area under the Windsor Framework and go through Union OSS at the UK's 20%; **digital services to NI are out of scope entirely**. One postcode, two answers, decided by which product is in the parcel |
