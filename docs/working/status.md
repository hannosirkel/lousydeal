...
| Printful webhook subscribed, and its secret captured | **no** | **operator action.** `POST /v2/webhooks` creates the configuration and returns the secret **once**; it goes wherever the API token goes, as `PRINTFUL_WEBHOOK_SECRET`. Without it the endpoint verifies nothing and therefore accepts nothing, which is the correct state rather than a broken one |
