...
| Merch exists in Medusa | **no** | `seed-merch.ts` defines the records and a `MerchSeedTarget` seam, and **nothing implements it** — no target, no command, no `predeploy` entry. P6 was ticked as though it did. **P6b** |
