...
`vitest.config.ts` and `scripts/validate`. `backend/` holds the Medusa backend's
environment reader so far; `storefront/` does not exist yet. The catalogue
declares `languages: [shell, typescript]` and `npm_project: true`.
